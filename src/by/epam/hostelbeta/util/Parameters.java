package by.epam.hostelbeta.util;

// TODO: Auto-generated Javadoc
/**
 * The Class Parameters.
 */
public class Parameters {
	
	/** The Constant FULLNAME. */
	// for User
	public static final String FULLNAME = "fullname";
	
	/** The Constant PASSPORT. */
	public static final String PASSPORT = "passport";
	
	/** The Constant EMAIL. */
	public static final String EMAIL = "email";
	
	/** The Constant PHONE. */
	public static final String PHONE = "phone";
	
	/** The Constant LOGIN. */
	public static final String LOGIN = "login";
	
	/** The Constant PASSWORD. */
	public static final String PASSWORD = "password";
	
	/** The Constant ROLE. */
	public static final String ROLE = "role";

	/** The Constant NAME. */
	// for Hostel
	public static final String NAME = "name";
	
	/** The Constant COUNTRY. */
	public static final String COUNTRY = "country";
	
	/** The Constant CITY. */
	public static final String CITY = "city";
	
	/** The Constant ADDRESS. */
	public static final String ADDRESS = "address";
	
	/** The Constant CURRENCY. */
	public static final String CURRENCY = "currency";
	
	/** The Constant STANDART_PRICE. */
	public static final String STANDART_PRICE = "standart_price";
	
	/** The Constant DESCRIPTION. */
	public static final String DESCRIPTION = "description";

	/** The Constant ROOM_TYPE. */
	// for Room
	public static final String ROOM_TYPE = "roomType";
	
	/** The Constant BEDS_NUMBER. */
	public static final String BEDS_NUMBER = "bedsNumber";

	/** The Constant LOCALE. */
	public static final String LOCALE = "locale";
	
	/** The Constant PAGE. */
	public static final String PAGE = "page";
	
	/** The Constant CONFIG. */
	public static final String CONFIG = "config";
	
	/** The Constant COMMAND. */
	public static final String COMMAND = "command";
	
	/** The Constant DEFAULT. */
	public static final String DEFAULT = "Default";
	
	/** The Constant LOCALE_MANAGER. */
	public static final String LOCALE_MANAGER = "locale_manager";
	
	/** The Constant USER_DATA_LOGIN. */
	public static final String USER_DATA_LOGIN = "user_data.login";
	
	/** The Constant USER_DATA_ROLE. */
	public static final String USER_DATA_ROLE = "user_data.role";
	
	/** The Constant INCORRECT_LOGIN_MESSAGE. */
	public static final String INCORRECT_LOGIN_MESSAGE = "error.incorrect_login_password";
	
	/** The Constant ERROR_LOGIN_PASS_MESSAGE. */
	public static final String ERROR_LOGIN_PASS_MESSAGE = "errorLoginPassMessage";
	
	/** The Constant ERROR_REGISTRATION_MESSAGE. */
	public static final String ERROR_REGISTRATION_MESSAGE = "errorRegistrationMessage";
	
	/** The Constant LOGIN_EXISTS_MESSAGE. */
	public static final String LOGIN_EXISTS_MESSAGE = "error.login_exists";
	
	/** The Constant HOSTEL_LIST. */
	public static final String HOSTEL_LIST = "hostelList";
	
	/** The Constant NO_OF_PAGES. */
	public static final String NO_OF_PAGES = "noOfPages";
	
	/** The Constant CURRENT_PAGE. */
	public static final String CURRENT_PAGE = "currentPage";
	
	/** The Constant PAGE_NUMBER. */
	public static final String PAGE_NUMBER = "pageNumber";
	
	/** The Constant USER_ID. */
	public static final String USER_ID = "userId";
	
	/** The Constant ORDER_LIST. */
	public static final String ORDER_LIST = "orderList";
	
	/** The Constant LOGIN_NOT_AVAILABLE. */
	public static final String LOGIN_NOT_AVAILABLE = "label.login_not_available";
	
	/** The Constant LOGIN_CHECK_ERROR. */
	public static final String LOGIN_CHECK_ERROR = "label.login_check_error";
	
	/** The Constant INVALID_DATA. */
	public static final String INVALID_DATA = "error.invalid_data";
	
	/** The Constant ERROR_STACKTRACE. */
	public static final String ERROR_STACKTRACE = "errorStackTrace";
	
	/** The Constant HOSTEL_ID. */
	public static final String HOSTEL_ID = "hostelId";
	
	/** The Constant ROOM_LIST. */
	public static final String ROOM_LIST = "roomList";
	
	/** The Constant HOSTEL_NAME. */
	public static final String HOSTEL_NAME = "hostelName";
	
	/** The Constant ORDER_ID. */
	public static final String ORDER_ID = "orderId";

	/** The Constant RELOAD. */
	public static final String RELOAD = "reload";
	
	/** The Constant OPERATION_SUCCESS. */
	public static final String OPERATION_SUCCESS = "label.operation_success";
	
	/** The Constant OPERATION_ERROR. */
	public static final String OPERATION_ERROR = "label.operation_error";
	
	/** The Constant COUNTRY_LIST. */
	public static final String COUNTRY_LIST = "countryList";
	
	/** The Constant CURRENCY_LIST. */
	public static final String CURRENCY_LIST = "currencyList";
	
	/** The Constant HOSTEL. */
	public static final String HOSTEL = "hostel";
	
	/** The Constant ERROR_ADD_HOSTEL_MESSAGE. */
	public static final String ERROR_ADD_HOSTEL_MESSAGE = "errorAddHostelMessage";
	
	/** The Constant ERROR_ADD_ROOM_MESSAGE. */
	public static final String ERROR_ADD_ROOM_MESSAGE = "errorAddRoomMessage";
	
	/** The Constant ROOM_ID. */
	public static final String ROOM_ID = "roomId";
	
	/** The Constant ROOM_TYPE_LIST. */
	public static final String ROOM_TYPE_LIST = "roomTypeList";
	
	/** The Constant ROOM. */
	public static final String ROOM = "room";
	
	/** The Constant ROOM_ID_EXISTS_MESSAGE. */
	public static final String ROOM_ID_EXISTS_MESSAGE = "error.room_id_exists";
	
	/** The Constant BAN. */
	public static final String BAN = "ban";
	
	/** The Constant REGULAR. */
	public static final String REGULAR = "regular";
	
	/** The Constant USER_LIST. */
	public static final String USER_LIST = "userList";
	
	/** The Constant IN_DATE. */
	public static final String IN_DATE = "inDate";
	
	/** The Constant OUT_DATE. */
	public static final String OUT_DATE = "outDate";
	
	/** The Constant REGISTRATION. */
	public static final String REGISTRATION = "registration";
	
	/** The Constant ERROR_MESSAGE. */
	public static final String ERROR_MESSAGE = "errorMessage";
	
	/** The Constant DATE_BEFORE_TODAY. */
	public static final String DATE_BEFORE_TODAY = "error.date_before_today";
	
	/** The Constant IN_DATE_AFTER_OUT. */
	public static final String IN_DATE_AFTER_OUT = "error.date_in_after_out";
	
	/** The Constant BOOKING. */
	public static final String BOOKING = "booking";
	
	/** The Constant ROOM_IS_BOOKED_SUCCESSFUL. */
	public static final String ROOM_IS_BOOKED_SUCCESSFUL = "label.room_booked";
	
	/** The Constant ROOM_BOOKING_FAIL. */
	public static final String ROOM_BOOKING_FAIL = "label.room_booking_fail";
	
	/** The Constant HASH. */
	public static final String HASH = "hash";
	
	/** The Constant FORM_HASH. */
	public static final String FORM_HASH = "formHash";
	
	/** The Constant DISCOUNT. */
	public static final String DISCOUNT = "discount";
	
	/** The Constant CANT_CANCEL_ORDER. */
	public static final String CANT_CANCEL_ORDER = "error.cant_cancel_order";
	
	/** The Constant MIN_PRICE. */
	public static final String MIN_PRICE = "minPrice";
	
	/** The Constant MAX_PRICE. */
	public static final String MAX_PRICE = "maxPrice";
	
	/** The Constant INVALID_PRICES. */
	public static final String INVALID_PRICES = "error.invalid_prices";
	
	/** The Constant MAIL. */
	public static final String MAIL = "mail";
	
	/** The Constant SESSION_USER. */
	public static final String SESSION_USER = "sessionUser";
	
	/** The Constant IS_SEARCH. */
	public static final String IS_SEARCH = "isSearch";
	
	/** The Constant CANT_DELETE_HOSTEL. */
	public static final String CANT_DELETE_HOSTEL = "error.cant_delete_hostel";
	
	/** The Constant CANT_DELETE_ROOM. */
	public static final String CANT_DELETE_ROOM = "error.cant_delete_room";
	
	/** The Constant CANT_REJECT_ORDER. */
	public static final String CANT_REJECT_ORDER = "error.cant_reject_order";
	
	/** The Constant CAUSE. */
	public static final String CAUSE = "cause";
}